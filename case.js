...
const { formatp, formatDate, getTime, isUrl, sleep, clockString, runtime, fetchJson, getBuffer, jsonformat, format, webApi, parseMention, uploadToUguu, generateProfilePicture, getRandom, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, generateToken, batasiTeks, randomText, isEmoji, getAllHTML, fetchBuffer, link, GIFBufferToVideoBuffer } = require('./lib/function')
module.exports = async (Ramah, m) => {
try {
const from = m.key.remoteJid
const PhoneNumber = await Ramah.decodeJid(Ramah.user.id)
...
const { smsg, msg, split, fetchJson, getBuffer, fetchBuffer, getGroupAdmins, isUrl, hitungmundur, sleep, clockString, checkBandwidth, runtime, tanggal, getRandom } = require('./lib/myfunc')
...
const groupMetadata = isGroup? await Ramah.groupMetadata(m.chat) : {}
let participant_bot = isGroup ? groupMetadata.participants.find((v) => v.if == botNumber) : {}
...
/*const isGroup = m.chat.endsWith('@g.us')
/*const groupMetadata = m.isGroup ? await Ramah.groupMetadata(from).catch(e => {}) : ''*/
...
try {
ppuser = await Ramah.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
case 'ramah': case '🐥': case 'r': case 'han':{
...
「 *⿈ 𝐑𝐀𝐌𝐀𝐇 𝐀𝐋𝐏𝐇𝐀* 」`
Ramah.sendMessage(from, { image: { url: `https://files.catbox.moe/hsi1o1.jpg` }, caption: teks, fileLength: 10, contextInfo: { mentionedJid:[sender], forwardingScore: 9999, isForwarded: true }}, { quoted: m })
}
break
...
case 'tools-menu': case 'downloader': {
let kntll = fs.readFileSync('./lib/menu.mp3')
    Ramah.sendMessage(m.chat, { audio: kntll, mimetype: "audio/mp4", ptt: true }, { quoted: m })
Ramah.sendMessage(from, {react: {text: randomReaction, key: m.key}})
let teks = `
...
case'xnxxs': case 'xnxxsearch': {
 if (!isCreator) return tolak(m.OnlyOwner)
 if (m.isGroup) return warning("For the Convenience of Other Members, Please Use This Command in Private Chat")
 if (!text) return m.reply(`• *Example :* ${prefix + command} stepmoms`)
 load()
...
Ramah.sendMessage(m.chat, {text: `*VERTEX MD HAS BEEN RUNNING* :\n*${runtime(process.uptime())}*

> OWNER   :「 *⿈ 𝐑𝐀𝐌𝐀𝐇 𝐀𝐋𝐏𝐇𝐀* 」
> CHANNEL : *https://whatsapp.com/channel/0029VakZxvW8Pgs8FIrFQx0P*
> GROUP   : *https://chat.whatsapp.com/E1eQ4voBguTBBvey2KpMyi*`}, 
{quoted: m})
break

case 'wastalk': case 'nowa': {
 let num = m.quoted?.sender || m.mentionedJid?.[0] || text
 if (!num) return m.reply(`Ex: ${prefix + command} @tag / 256xxx`)
 num = num.replace(/\D/g, '') + '@s.whatsapp.net'
 if (!(await Ramah.onWhatsApp(num))[0]?.exists) throw 'User not exists'
 let img = await Ramah.profilePictureUrl(num, 'image').catch(_ => 'https://btch.pages.dev/file/70e8de9b1879568954f09.jpg')
 let bio = await Ramah.fetchStatus(num).catch(_ => {})
 let name = await Ramah.getName(num)
 let business = await Ramah.getBusinessProfile(num)
 let format = PhoneNumber(`+${num.split('@')[0]}`)
 ...
 img ? await Ramah.sendMessage(m.chat, {
 image: {
 url: img
 },
 caption: res,
...
return m.reply('The folder already exists at that location!');
}
fs.RamahSync(folderPath, { recursive: true });
m.reply(`DONE ✅ SUCCESSFULLY${folderPath}`);
} catch (error) {
...
case 'drakor': {
 if (!text) return m.reply("VERTEX SEARCH: Drama The Red Sleeve")
 m.reply(`_🔎Mencari ${text}_`);
 try {
...
const BlackBox = {
 ...
 let { key } = await Ramah.sendMessage(m.chat, { text: "..." });
 try {
 // Pastikan minimal ada teks
 if (!text) return m.reply("Enter a question to answer!\n\n*Contoh:* Siapa presiden Indonesia?");
 ...
 await Ramah.sendMessage(m.chat, {
...
break
    ...
    case "menu": case "rmh": {
if (!isCreator) return reply(`RAMAH AS THE OWNER`)
let imgdana = await prepareWAMessageMedia({ image: fs.readFileSync("./thumb.png")}, { upload: Ramah.waUploadToServer })
...
buttons: [{
"name": "single_select",
"buttonParamsJson": `{ "title": "© VERTEX MD", "sections": [{ "title": " ⽶ | t.me/Ramadhanyusuf ", "highlight_label": \"VERTEX ᝄ\", "rows": [{ "header": "ALL MENU", "title": "̟", "id": ".ramah" },{ "header": "TOOLS MENU", "title": "̟", "id": ".tools-menu"},{ "header": "OTHER MENU", "title": "̟", "id": ".others"},{ "header": "OWNER MENU", "title": "̟", "id": ".owner-menu"},{ "header": "Click To Popup Menu", "title": "̟", "id": ".gc-menu"},{ "header": "ANIME MENU", "title": "̟", "id": ".anime-menu"},{ "header": "ISLAM MENU", "title": "̟", "id": ".islam-menu"}]}]}`
}]
})
}
]
...
await Ramah.relayMessage(m.chat, msgii.message, {messageId: msgii.key.id})
}
break

case "checkidch": {
if (!text) return m.reply("GET ON POINT, WHERE IS THE LINK?")
if (!text.includes("https://whatsapp.com/channel/")) return reply("LoL Bro invalid Link")
let result = text.split('https://whatsapp.com/channel/')[1]
let res = await Ramah.newsletterMetadata("invite", result)
let teks = `
* *Name :* ${res.name}
* *Total Follower :* ${res.subscribers}
`
return m.reply(teks)
}
break
...
case 'getcase':
if (!isCreator) return reply(m.onlyOwner)
const getCase = (cases) => {
...
case "toimage": {
if (!/webp/.test(mime) && !/audio/.test(mime)) return reply('please reply to a steacker') 
await reply(msg.wait)
let media = await Ramah.downloadAndSaveMediaMessage(qmsg)
let ran = `${makeid}.png`
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return err
let buffer = fs.readFileSync(ran)
Ramah.sendMessage(m.chat, {image: buffer}, {
quoted: m})
fs.unlinkSync(ran)
})
}
...
if (!isAdmin && !isCreator) return KleeReply(msg.admin)
await Ramah.groupSettingUpdate(m.chat, 'announcement')
...
case 'rplay' : {
 ...
 await reply("「 *⿈ 𝐑𝐀𝐌𝐀𝐇 𝐀𝐋𝐏𝐇𝐀* 」> *VERTEX MD ON 🔍, Please wait...*");
 try {
...
if (!isGroup) return reply(msg.group)
if (!isBotAdmin) return reply(msg.adminbot)
if (!isAdmin && !isCreator) return reply(msg.admin)
if (text || m.quoted) {
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Ramah.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => Ramah.sendMessage(m.chat, {text: `Successfully Dispensing @${users.split("@")[0]} From this group`, mentions: [`${users}`]}, {quoted: m})).catch((err) => reply(err.toString()))
} else return reply('lol/@tag')}
break
...
await Ramah.groupSettingUpdate(m.chat, 'not_announcement')
reply("*_Vertex Md Successfully change the group's settings to allow sending of messages_*")
}
break
/*
case "toimage": {
...
case "tourl": {
if (!/image/.test(mime)) return reply(example("PLEASE REPLY TO AN IMAGE TO EASE THE WORK OF VERTEX MD"))
await reply(msg.wait)
var fotonya = await Ramah.downloadAndSaveMediaMessage(qmsg)
var urlimage = await telegraPh(fotonya)
await reply(`WOW HERE IS YOUR Link :\n${urlimage}`)
await fs.unlinkSync(photo)
}
break

case 'chatbot': {
const microsoftCopilotNumber = '256702025571@s.whatsapp.net';
if (!text) {
return reply("Silakan masukkan pesan yang ingin dikirim.");
}
await Ramah.sendMessage(microsoftCopilotNumber, {
text: text
});
m.reply("Message successfully forwarded to Microsoft Copilot. Please wait for a reply.");
if (global.responseListener) {
Ramah.ev.off('messages.upsert', global.responseListener);
}
...
Ramah.ev.on('messages.upsert', global.responseListener);
}
break
...
case 'rmdfile': {
 ...
 return m.reply('*Oh, the size is so big, I donot want to haha*');
 }
...
▢ *UploadAt*: ${date}`;
 ...
 Ramah.sendMessage(m.chat, { document : { url : fileurl}, fileName : filename, caption: caption, mimetype: filetype }, { quoted : m });
}
break

case "setnamebot": {
if (!isCreator) return reply(msg.owner)
if (!text) return reply(example('Vertex Md'))
Ramah.updateProfileName(text)
...
Ramah.sendMessage(m.chat, { video: { url: anu.no_watermark }, mimetype: 'video/mp4', caption: anu.title }, { quoted : m })
} catch (error) {
reply('Error :v')
}
}
...
try {
const contact = (await Ramah.onWhatsApp(recipient.split('@')[0]))[0] || {};
if (!contact) return m.reply('Kontak tidak ditemukan di WhatsApp.');
const message = `Hi, kamu dapet kiriman fitur nih!\n\n${caseCode}`;
await Ramah.sendMessage(recipient, { text: message }, { quoted: m });
...
Ramah.sendMessage(m.chat, {video: {url: kanjutgedeu.media}, caption: mess.success}, {quoted: m})
}
break
...
if (!q) return m.reply(`quwry?`)
    async function prodia(query) {
        ...
        try {            // wm avz
            ...
            Ramah.sendMessage(m.chat, { image: { url: imageUrl }, caption: '_donee ketua_.' });
        } catch (error) {
            m.reply('Gagal');
        }
    }    
...
`
Ramah.sendMessage(m.chat, {text: ahwoi}, {quoted: m})
} catch (error) {
...
Ramah.sendMessage(m.chat, {image: {url: haiij}.url}, {quoted: m})
} catch (error) {
  reply(`error bang`)
}
}
...
try {
    ...
    let no = 1
    for (let i of result) {
        Ramah.sendMessage(m.sender, {image: {url: i}, caption: `Gambar ke - ${no++}`}, {quoted: m})
}
} catch (error) {
 m.reply(error)
}
...
${global.namabot}
`
Ramah.sendMessage(m.chat, {image: {url: knut.imgSrc}, caption: bjirpo}, {quoted: m})
}
break
case 'tomoe': {
    if (!text) {
...
const sendToGemini = async (prompt) => {
    ...
    try {
        ...
        if (response) {
            ...
            await Ramah.sendMessage(m.chat, { text: message }, { quoted: m });
        } else {
            await Ramah.sendMessage(
                m.chat,
...
const waifut = "https://" + waifud.file_url
  Ramah.sendMessage(from, { image: { url: waifut }, caption: `Hell nah\n\nSize: ${waifud.file_size}` }, { quoted: m })
}
break
case 'elonmu-news': {
...
(`https://elonmu.sh/api`)
  ...
  `
  Ramah.sendMessage(from, {image: {url: arsipp.urlImage}, caption: bluee}, {quoted: m})
}
break
case 'tebakumur': {
...
(`https://api.agify.io/?name=${text}`)
  ...
  Umur: ${mmeeg.age}
  `
  await Ramah.sendMessage(from, {text: qour}, {quoted: m})
}
break
case 'freetogame': {
...
let brak = pickRandom(kocakk)
  ...
  `
  await Ramah.sendMessage(from, {image: {url: brak.thumbnail}, caption: gamefree}, {quoted: m})
}
break
case 'yesorno': {
...
(`https://yesno.wtf/api`)
  ...
  let buttonMessage = {
            ...
            Ramah.sendMessage(m.chat, buttonMessage,{ quoted:m })
}
break
case 'doaharian': {
var pripayerr = [
...
// wm avz
  Ramah.sendImageAsSticker(m.chat, buffer, m, { packname: global.pack, author: global.author });
}
break
...
Ramah.sendMessage(m.chat, {image: {url: yeha }, caption: mess.success}, {quoted: m})
}
break
case 'putar': {
...
let previousResult = null
            ...
            message: {
                ...
                interactiveMessage: proto.Message.InteractiveMessage.create({
                    ...
                    header: proto.Message.InteractiveMessage.Header.create({
                        ...
                        ...await prepareWAMessageMedia({ image: buffer }, { upload: Ramah.waUploadToServer })
                    }),
    ...
    return await Ramah.relayMessage(m.chat, msgs.message, {})
}
break
case 'sfile': {
...
case 'songgen': {
    ...
    const SongGenerator = async (prompt) => {
        try {
            const { key } = await Ramah.sendMessage(m.chat, { text: "Please wait.." }, { quoted: m });
                    ...
                    try {
                        ...
                        if (eventData && eventData[1]) {
                            ...
                            switch (data.status) {
                                case 'queueing':
                                case 'generating':
                                    Ramah.sendMessage(m.chat, { text: data.msg, edit: key });
                                    break;
                                case 'failed':
                                ...
                                case 'success':
                                    ...
                                    break;
                                default:
                                    Ramah.sendMessage(m.chat, { text: "Unknown status: " + data, edit: key });
                            }
                        }
                    } catch (e) {
                        Ramah.sendMessage(m.chat, { text: "Error processing chunk: " + e.message, edit: key });
                        response.data.destroy();
                        reject(e);
                    }
                });
                response.data.on('error', (err) => {
                    Ramah.sendMessage(m.chat, { text: "Stream error: " + err.message, edit: key });
                    reject(err);
                });
            });
        } catch (error) {
            Ramah.sendMessage(m.chat, { text: "Error: " + error.message, edit: key });
            throw error;
        }
    };
    try {
        const data = await SongGenerator(text);
        await Ramah.sendPresenceUpdate('recording', m.chat);
        await Ramah.sendMessage(m.chat, { audio: { url: data.result.audioUrl }, mimetype: "audio/mpeg", ptt: true }, { quoted: m });
        let doc = {
            ...
            contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    mediaType: 2,
                    mediaUrl: `http://wa.me/256788028745/${Math.floor(Math.random() * 100000000000000000)}`,
        ...
        await Ramah.sendMessage(m.chat, doc, { quoted: m });  
    } catch (err) {
        console.error(err);
    }
    }
    break;
                case 'userjid':{
          	if(!isCreator) return reply(mess.OnlyOwner)
        const groupMetadata = m.isGroup ? await Ramah.groupMetadata(m.chat).catch((e) => {}) : ""
		const participants = m.isGroup ? await groupMetadata.participants : ""
...
case 'bass': case 'blown': case 'deep': case 'earrape': case 'fast': case 'fat': case 'nightcore': case 'reverse': case 'robot': case 'slow': case 'smooth': case 'squirrel':
                ...
                if (/audio/.test(mime)) {
                let media = await Ramah.downloadAndSaveMediaMessage(quoted)
                let ran = getRandom('.mp3')
                exec(`ffmpeg -i ${media} ${set} ${ran}`, (err, stderr, stdout) => {
                fs.unlinkSync(media)
                if (err) return reply(err)
                let buff = fs.readFileSync(ran)
                Ramah.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : m })
                fs.unlinkSync(ran)
                })
...
break
	...
	await Ramah.sendMessage(m.chat,{sticker: nah},{quoted:m})
...
// wm avs
    out.on('finish', async () => {
      await Ramah.sendMessage(from, { image: { url: outputPath }, caption: '_done nih_.', fileName: 'quote.png' }, { quoted: m });
...
break
case 'mediafire': {
	if (!args[0]) return m.reply(`Insert url mediafire\nContoh: ${prefix + command} https://www.mediafire.com/××××××××`);
  ...
  reply(bjirrbangg)
  await Ramah.sendMessage(m.chat, { document: { url: url }, fileName: title, mimetype: type }, { quoted: m })
}
break
        case 'vv': {
...
if (/video/.test(type)) {
await Ramah.sendMessage(m.chat, { video: buffer, caption: quotedType.caption })
} else if (/image/.test(type)) {
await Ramah.sendMessage(m.chat, { image: buffer, caption: quotedType.caption })
}
}
break
        case 'pixeldrain': {
...
async function scrapeListData(url) {
	...
	if (fileIdMatch) {
		...
		Ramah.pixpix[m.chat] = {
			...
			timeout: setTimeout(
				() => {
					Ramah.sendMessage(m.chat, { delete: key });
					setTimeout(() => {
						delete Ramah.pixpix[m.chat];
					}, 60 * 1000);
				},
				5 * 60 * 1000,
			),
...
try {
  ...
  reply(bjirrbang)
  await Ramah.sendMessage(m.chat, { document: { url: kanjuttgede.downloadUrl }, fileName: kanjuttgede.fileName, mimetype: kanjuttgede.mimetype }, { quoted: m })
} catch (error) {
  m.reply(`${error.message}`)
}
}
...
try {
  ...
  Ramah.sendMessage(m.chat, {text: hannpler}, {quoted: m})
} catch (error) {
  m.reply("Error bang")
}
}
...
case 'pixiv': {
  ...
  await Ramah.sendMessage(m.chat, { image: hannunibakwan, caption: "Done" }, { quoted:m });
}
break
        case 'playstore': {
...
break
        case "qc": {
if (!quoted){
const getname = await Ramah.getName(mentionUser[0])
const json = {
"type": "quote",
"format": "png",
...
Ramah.sendImageAsSticker(from, buffer, m, opt)
});
} else if (q) {
const json = {
"type": "quote",
...
class RetaTube extends API {
    ...
    try {
        ...
        if (result.videoLinks.length > 0) {
            ...
            await Ramah.sendMessage(m.chat, { video: { url: video.url }, caption: videoMessage }, { quoted: m });
        } else {
...
`
            let menst = [top1, top2, top3, top4, top5]
                    await Ramah.sendText(m.chat, jawab, m, {mentions: menst})
            }
            break
            case 'del': case 'd':{
Ramah.sendMessage(m.chat,
...
let encmedia = await Ramah.sendImageAsSticker(m.chat, media, m, { packname: global.pack, author: global.author + "\n".repeat(1000) })
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Send/Reply Images/Videos/Gifs With Captions ${prefix+command}\nVideo Duration 1-9 Seconds')
let media = await quoted.download()
let encmedia = await Ramah.sendVideoAsSticker(m.chat, media, m, { packname: global.pack, author: global.author + "\n".repeat(1000) })
} else {
...
case 'lahelu': {
	...
	if(/^video/i.test(data.media)) return await Ramah.sendMessage(m.chat, { video: { url: 'https://cache.lahelu.com/'+data.media }, caption: data.title, viewOnce: true }, { quoted: m }) 
	if(/^image/i.test(data.media)) return await Ramah.sendMessage(m.chat, { image: { url: 'https://cache.lahelu.com/'+data.media }, caption: data.title, viewOnce: true }, { quoted: m }) 
	}
break
    case 'gemini': {
...
h		}
  ...
  try {
    ...
    await Ramah.sendMessage(m.chat, { video: { url: result }, caption: "Done" }, { quoted: m });
  } catch (error) {
    console.log(error);
...
// wm avs
    nyariGambar(text).then(images => {
        ...
        Ramah.sendMessage(m.chat, { image: { url: imageAvz }, caption: `*Query* : ${text}\n*Media Url* : ${imageAvz}` }, { quoted: m })
    }).catch(error => {
...
case 'kbbi-kemdikbud': {
 ...
 // wm avs        
        Ramah.sendMessage(m.chat, { text: `${definition}` }, { quoted: m })
    }).catch(error => {
...
//avosky
  if (result && result.result_url) {
    ...
    Ramah.sendMessage(m.chat, message);
  } else {
    m.reply('err.');
  }
}
...
const readmore = more.repeat(4001)
    ...
    if (!l) l = ''
    if (!r) r = ''
    Ramah.sendMessage(m.chat, {text: l + readmore + r}, {quoted: m})
}
break;
...
try {
    ...
    const cap = `HERE IS THE VIDEO`;
            ...
            await Ramah.sendMessage(m.chat, { video: { url: mediaURL }, caption: cap }, { quoted: m });
...
${global.namabot}`;
await Ramah.sendMessage(m.chat, { video: { url: maximus.video[0] }, caption: caption })
await Ramah.sendMessage(m.chat, { audio: { url: maximus.audio[0] }, mimetype: "audio/mpeg", ptt: true }, { quoted: m })
      } catch (e) {
		console.log(e)
		reply(e)
	}
...
break
  ...
  await Ramah.relayMessage(m.chat, bot.message, {
...
if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
			reply(m.wait)
			axios.get(`https://www.dark-yasiya-api.site/download/ytmp4?url=https://youtube.com/watch?v=OJExUP3-r8w&quality=1080p=${args[0]}`)
				.then(({ data }) => {
